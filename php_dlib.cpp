...
/* If you declare any globals in php_dlib.h uncomment this:
ZEND_DECLARE_MODULE_GLOBALS(php_dlib)
*/
...
PHP_INI_BEGIN()
    STD_PHP_INI_ENTRY("php_dlib.global_value",      "42", PHP_INI_ALL, OnUpdateLong, global_value, zend_php_dlib_globals, php_dlib_globals)
    STD_PHP_INI_ENTRY("php_dlib.global_string", "foobar", PHP_INI_ALL, OnUpdateString, global_string, zend_php_dlib_globals, php_dlib_globals)
PHP_INI_END()
*/
/* }}} */


/* {{{ php_dlib_init_globals
 */
/* Uncomment this function if you have INI entries
static void php_dlib_init_globals(zend_php_dlib_globals *php_dlib_globals)
{
	php_dlib_globals->global_value = 0;
	php_dlib_globals->global_string = NULL;
}
*/
/* }}} */
...
inline face_recognition* get_face_recognition_from_std(zend_object *obj) {
    face_recognition_object *intern =
        (face_recognition_object *)((char *)obj - XtOffsetOf(face_recognition_object, std));
...
{
    face_recognition_object *intern =
        (face_recognition_object *)ecalloc(1, sizeof(face_recognition_object));

    zend_object_std_init(&intern->std, class_type);
    object_properties_init(&intern->std, class_type);

    intern->std.handlers = &face_recognition_obj_handlers;

    intern->cpp = new face_recognition();
...
{
    face_recognition_object *intern =
        (face_recognition_object *)((char *)object - XtOffsetOf(face_recognition_object, std));
...
PHP_MINIT_FUNCTION(php_dlib)
{
	...
	face_recognition_obj_handlers.offset = XtOffsetOf(face_recognition_object, std);
 ...
 */
PHP_MSHUTDOWN_FUNCTION(php_dlib)
{
...
/* {{{ PHP_RINIT_FUNCTION
 */
PHP_RINIT_FUNCTION(php_dlib)
{
#if defined(COMPILE_DL_PHP_DLIB) && defined(ZTS)
 ...
 */
PHP_RSHUTDOWN_FUNCTION(php_dlib)
{
	return SUCCESS;
}
/* }}} */
...
/* {{{ PHP_MINFO_FUNCTION
 */
PHP_MINFO_FUNCTION(php_dlib)
{
	char buf[32];

	php_info_print_table_start();
	php_info_print_table_header(2, "php_dlib support", "enabled");
	php_info_print_table_row(2, "php_dlib extension version", PHP_DLIB_VERSION);
...
/* {{{ php_dlib_functions[]
 *
 * Every user visible function must have an entry in php_dlib_functions[].
 */
const zend_function_entry php_dlib_functions[] = {
	...
	PHP_FE_END	/* Must be the last line in php_dlib_functions[] */
};
/* }}} */

/* {{{ php_dlib_module_entry
 */
zend_module_entry php_dlib_module_entry = {
	STANDARD_MODULE_HEADER,
	PHP_DLIB_NAME,
	php_dlib_functions,
	PHP_MINIT(php_dlib),
	PHP_MSHUTDOWN(php_dlib),
	PHP_RINIT(php_dlib),		/* Replace with NULL if there's nothing to do at request start */
	PHP_RSHUTDOWN(php_dlib),	/* Replace with NULL if there's nothing to do at request end */
	PHP_MINFO(php_dlib),
	PHP_DLIB_VERSION,
...
#ifdef COMPILE_DL_PHP_DLIB
#ifdef ZTS
ZEND_TSRMLS_CACHE_DEFINE()
#endif
ZEND_GET_MODULE(php_dlib)
#endif
