...
static inline face_recognition_object *php_face_recognition_from_obj(zend_object *obj) {
    return (face_recognition_object*)((char*)(obj) - XtOffsetOf(face_recognition_object, std));
}
...
PHP_METHOD(FaceRecognition, __construct)
{
    ...
    face_recognition_object *fr = Z_FACE_RECOGNITION_P(getThis());
    ...
    try {
        ...
        fr->cpp->net = new anet_type;
        deserialize(face_recognition_model_path) >> *(fr->cpp->net);
    } catch (exception& e) {
...
std::vector<matrix<rgb_pixel>> php_dlib_jitter_image(
...
PHP_METHOD(FaceRecognition, computeDescriptor)
{
    ...
    std::vector<point> parts_points(parts_count);
    ...
    try {
        ...
        if (num_jitters <= 1) {
            std::vector<matrix<float,0,1>> face_descriptors = fr->cpp->net->operator()(face_chips, 16);
            face_descriptor = face_descriptors[0];
        } else {
            matrix<rgb_pixel>& face_chip = face_chips[0];
            face_descriptor = mean(mat(fr->cpp->net->operator()(php_dlib_jitter_image(face_chip, num_jitters, fr->cpp->rnd), 16)));
        }
